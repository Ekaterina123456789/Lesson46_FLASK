...
def show_product(product_id):
    ...
    title, product = dbase.get_product(product_id)
    if not title:
        abort(404)

    return render_template('product.html', title=title, product=product, menu=dbase.get_objects('mainmenu'))
...
def catalog():
    db_con = connect_db()
    dbase = DataBase(db_con)
    return render_template('catalog.html', title='Каталог', menu=dbase.get_objects('mainmenu'))
